...
The data we used was from kenpom.com, containing team-level statistics for NCAA Division I basketball.
...
The causal diagram illustrates the assumed relationships between the variables in our analysis. The primary goal is to model the probability of a team reaching the Final Four (Final_Four) based on core performance metrics.

- Predictors Included: We included AdjOE, AdjDE, and AdjTempo as direct predictors of Final_Four. These variables represent adjusted offensive efficiency, adjusted defensive efficiency, and adjusted tempo, respectively.
...
```{r}
# Treat NA Final Four appearances as FALSE
dat_all_teams <- data %>%
  mutate(final_four = if_else(is.na(final_four), 0, final_four)) %>%
  mutate(final_four = final_four >= 1) %>%
  drop_na(seed)
...
## Prior Predictive Distribution of all teams included in the NCAA tournament
...
```{r}
# Extract the summary from the Stan fit object
summary_fit <- summary(fit_no_cat)$summary

# Print the first 10 rows of the summary
print(summary_fit[1:10, ])
```
...
```{r}
# Extract the summary from the Stan fit object
summary_fit <- summary(fit_final_four_only)$summary
